...
class Customer(models.Model):
    ...
    product_product_id = fields.Many2many("product.template", relation='product_vendor_id_rel', column1='brand_id', column2='prod_id', string="Products list")
    is_exported = fields.Boolean('Synced In Woocommerce', default=False)
    b2b = fields.Selection([('b2b', 'B2B'), ('b2c', 'B2C')], 'Customer Type', default='b2c')
    points = fields.Float("Greenelly Points", default=0)
    user_lvl = fields.Integer("User Level", default=0)
    friends_count = fields.Integer("Friends Count", default=0)

    orders = fields.Integer("Orders", compute="_depends_count_orders", store=True)
    ...
    def _depends_count_orders(self):
        for record in self:
            sale_orders = self.env['sale.order'].sudo().search_count([('partner_id', '=', record.id)])
            if sale_orders:
                record.orders = sale_orders
            else:
                record.orders = 0
    ...
    def import_customer(self, instance_id, c_id=None):
        _logger.info("Importing Customer")
        location = instance_id.url
        cons_key = instance_id.client_id
        sec_key = instance_id.client_secret
        version = 'wc/v3'
        ...
        while page > 0:
            try:
                if not c_id:
                    data = wcapi.get(url, params={'orderby': 'id', 'order': 'desc', 'role': 'all', 'per_page': 100, 'page': page})
                    page += 1
                else:
                    url = url + "/" + str(c_id)
                    data = wcapi.get(url)
            ...
            if data.status_code == 200 and data.content:
                ...
                if parsed_data:
                    for ele in parsed_data:
                        ...
                        if ele.get('id') == 808:
                            _logger.info(ele.get("meta_data"))

                        if ele.get("meta_data"):
                            for meta in ele.get("meta_data"):
                                if meta.get("key") == "balance":
                                    dict_p['points'] = float(meta.get("value"))
                                if meta.get("key") == "friends_count":
                                    dict_p['friends_count'] = int(meta.get("value"))
                                if meta.get("key") == "user_lvl":
                                    dict_p['user_lvl'] = int(meta.get("value"))
                        if ele.get('id') == 808:
                            _logger.info(dict_p)
