...
class WooInstance(models.Model):
    ...
    products_to_parse = fields.Char("Products to update", default="")

    wp_user = fields.Char('WP User login')
    wp_app_pass = fields.Char('WP Application password')
    woo_webhook_secret = fields.Char("Secret for WooCommerce order creation webhook")
    ...
    def instance_total_orders(self):
        ...
        view = self.env.ref('odoo_woo_commerce.action_sale_order_woo').read()[0]
